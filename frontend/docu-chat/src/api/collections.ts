...
import { CollectionIn, CollectionModelSchema, CollectionQueryInput, CollectionQueryOutput } from '../utils/types';
...
export const queryCollection = async (
    ...
    return axios.post<CollectionQueryOutput>(
        API_URL + "/api/collections/query",
        queryInput, 
        {headers}
    );
};

export const getMyCollections = async (authToken: string): Promise<
    AxiosResponse<CollectionModelSchema[]>
> => {
    return axios.get<CollectionModelSchema[]>(
        API_URL + "/api/collections/available", {
            headers: {
                Authorization: authToken,
            }
        });
 ...
 */
export const addFileToCollection = async (collectionId: number, file: File, authToken: string): Promise<any> => {
    ...
    try {
        const response: AxiosResponse<any> = await axios.post(`${API_URL}/api/collections/${collectionId}/add_file`, formData, {
            headers: {
                'Content-Type': 'multipart/form-data',
                Authorization: authToken,
            },
