...
import { CollectionIn } from '../utils/types';
...
describe('API', () => {
  ...
  it('should create a collection', async () => {
    ...
    const file2 = new File(['test content'], 'document2.txt', { type: 'text/plain' });
  ...
  it('should query a collection', async () => {
    ...
    const response = await axiosInstance.post(`collections/${collectionId}/query`, {
  ...
  it('should add a file to a collection', async () => {
    ...
    const response = await axiosInstance.post(`collections/${collectionId}/add_file`, formData);
    expect(response.status).toBe(200);
